...
./LibLoader --path=./PanoramaK.so 
```

После запуска программа напечатает в консоль список поддерживаемых трансиверов. Для ОС windows потребуется дополнительный шаг, 
